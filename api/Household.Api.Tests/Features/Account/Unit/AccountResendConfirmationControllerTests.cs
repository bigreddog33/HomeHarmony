...
public sealed class AccountResendConfirmationControllerTests
{
    ...
    public async Task ResendConfirmation_ForwardsRequestAndToken_AndReturnsServiceStatus(bool serviceResult, int expectedStatus)
    {
        ...
        var result = await controller.ResendConfirmation(request, source.Token);
    ...
    [Fact]
    public async Task ResendConfirmation_WithCancelledToken_PropagatesCancellation()
    {
        ...
        var exception = await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => controller.ResendConfirmation(request, source.Token));
